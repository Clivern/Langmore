...
They re append-only log files that hold the KV pairs along with some meta-information. A single `DWAL` instance could have many datafiles, out of which just one will be active and opened for writing, while the others are considered immutable and are only used for reads.
<br/>
<p align="center">
